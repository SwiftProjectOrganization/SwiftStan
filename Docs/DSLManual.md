...
richer `alist()` forms lower to — `Deterministic` and `VaryingPrior` — which is the
whole point of reading the generated driver.
...
*To be written.* Later sections will route the bernoulli, binomial, and
cross-classified (chimpanzees) examples from `UlamManual.md` through the DSL pipeline,
highlighting the additional DSL nodes (`Link`, `VectorPrior`) the richer models lower
to.
