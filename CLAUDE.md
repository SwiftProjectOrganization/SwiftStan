...
|---|---|---|
| sample | `<name>_output_[1..n].csv` | `<name>.samples.csv` |
...
| generated_quantities | `<name>_gq_[1..n].csv` | `<name>.generated_quantities.csv` |
...
- `optimize`, `pathfinder`, `laplace`, `generated_quantities`, `stansummary`, `csv2json`, `dsl2stan`, `alist2dsl`, `stancode`, `stan2alist`, `runinfo` — all use `OptionsLimited` (no `-I` or `-S`); `stan2alist` adds a `-f/--force` flag.
...
- `generated_quantities` — runs cmdstan's `generate_quantities` on draws from a prior `sample` run. Pre-flight-warns when `<name>.stan` has no `generated quantities` block; discovers chain files via `chainsFromRunInfo` (falls back to `chainOutputFiles`) and errors if none found. Shells out once per chain (`stanGenerated_Quantities`, `fitted_params=<chain>.csv`), then `getGenerated_QuantitiesResult` merges the per-chain raw `<name>_gq_[1..n].csv` files into `<name>.generated_quantities.csv`. Requires a prior `sample` run.
...
- `stan2alist` — the inverse of `stancode`. Reads `Results/<name>.stan` and writes a McElreath `alist()` to `Preliminaries/<name>.alist.R` via the reverse chain in `Ulam/Stan2Alist/`: `StanBlockParser` (line/block parser for the emitted-Stan subset → `StanProgram`; `rewriteContractLoops` inverts the forward single-assignment loop and the SUR two-statement loop before the `;`-splitter runs) → `StanToUlamModel` (reverse of `AlistToUlamModel` + `DataInference`; role inference reads block structure — likelihood = `~` over a non-index data vector, varying prior = `~` over a vector param whose size symbol matches an index column's `upper` bound, plain vector prior over a size with no matching index column, scalar prior = `~` over a `real` param, plus multivariate types `cholFactorCorr`/`arrayVector`/`covMatrix`/`matrix`; inverts `inv_logit`/`exp`/`logit` wrappers to `Link`s; rebuilds priors in parameter-declaration order) → `AlistTextEmitter` (`[Statement]` → alist text; renders `.bernoulli` as `dbinom(1, p)`). The Stan→`Distribution` reverse catalog (`distribution(fromStanName:args:)`, `mcElreathName(_:)`) lives in `Generator/DistributionCatalog.swift` next to the forward `name(_:)`/`args(_:)` so the tables can't drift — covers univariate and multivariate distributions (`multi_normal`, `multi_normal_cholesky`, `lkj_corr_cholesky`, `wishart`). Deliberately lossy per scope: declaration `<lower>`/`<upper>` constraints are dropped (re-derived by `AlistClassify` on the forward pass), `<offset>`/`<multiplier>` affine non-centering is stripped to the centred form, and `transformed *` blocks are dropped with a stderr warning. `generated quantities` is parsed (into `StanProgram.gqStatements`) and reverse-mapped: `<dist>_rng(...)` assignments → `Statement.generatedQuantity` → `<name> <- sim(d*(...))` alist text. Off-contract loops (`while`, 3+-statement bodies, recursive) fail loud. Refuses to overwrite an existing `.alist.R` without `--force`. Round-trip oracle: `alist → stancode → stan2alist → stancode` is byte-identical for vectorising models, contract-loop models (chimpanzees), and multivariate models via `Stan → [Statement] → stancode` (cafe Family A, SUR Family B).
...
Sub-packages:
- `AST/` — canonical AST types: `Statement` (`likelihood`/`prior` carry `truncation` and `useLpdf`; `varyingPrior` adds `indexedBy` and `countSymbol`; `vectorPrior` adds `length`; multivariate: `lkjCorrCholeskyPrior`, `varyingVectorPrior`, `matrixPrior`, `covMatrixPrior`, `wishartPrior`; `generatedQuantity` carries `name` + `distribution` for posterior-predictive simulation; `inits` carries a `[String: Double]` dict for start values), `Distribution` (catalog: `normal`, `bernoulli`, `binomial`, `beta`, `exponential`, `poisson`, `gamma`, `cauchy`, `lognormal`, `uniform`, `studentT`, `multivariateNormal`, `multivariateNormalCholesky`, `lkjCorrCholesky`, `wishart`, `orderedLogistic`, `orderedProbit`, `dirichlet`), `Constraints` (declaration-only `lower`/`upper`, distinct from `Truncation`), `LinkFunction`, `Expression` (raw-string wrapper in v1), `Truncation` (optional `lower`/`upper` + `.none`), `UlamModel` (top-level value; has both `@resultBuilder` and `(data:statements:)` inits), `ExpressionNode`.
- `Builder/` — `@resultBuilder StanModelBuilder` and DSL nodes: `Likelihood`, `Prior`, `VaryingPrior`, `VectorPrior`, `Link`, `Deterministic`, `Sim`, `Inits`. Initialisers carry defaulted `truncation:` and `useLpdf:`; `VaryingPrior` adds defaulted `countSymbol:` for overriding the auto-derived `N_<col>` cardinality variable; `VectorPrior` carries a `length:` cardinality symbol (for `vector[K] mu;`-style declarations under a `multi_normal` prior). `Sim("y_rep", .normal(...))` → `Statement.generatedQuantity`, emitting an `_rng` call in the Stan `generated quantities` block. `Inits(["mu": 178.0, "sigma": 25.0])` → `Statement.inits`, writing `Results/<name>.init.json` consumed by `StanSample` as `init=<path>`. All conform to `ModelStatement`.
...
- `Generator/` — `StanCodeGenerator` (public `stancode(_:) throws -> String`; `assemble(inferred:statements:)` prepends the traceability header), `BlockEmitter` (writes `data`/`parameters`/`model` blocks; routes through `vectorisationStrategy(...)` — canonical `a[group]` over known vector parameters + index columns vectorises, everything else throws `BlockEmitterError.loopEmissionRequired`; `generated_QuantitiesBlock(_:)` emits one `array[N] int/real <name> = <dist>_rng(<args>);` per `generatedQuantity` statement, returns `nil` when none so models without `sim()` stay byte-identical), `DataInference` (classifies symbols as data/parameter/derived, computes `N`, infers parameter constraints from prior truncations, records outcome bounds; tracks `vectorParameters` and `indexColumns` from `varyingPrior` cases; validates `sim()` references — rejects model-block locals with `generatedQuantityReferencesLocal` and non-scalar-`_rng` distributions with `unsupportedSimDistribution`), `DistributionCatalog` (R→Stan name + arg-order mapping, `isDiscrete` for `_lpmf`/`_lpdf`, `renderTruncation`, `renderConstraint`, `outcomeBounds`; also hosts the reverse Stan→`Distribution` catalog), recursive-descent `ExpressionParser`/`ExpressionLexer`.
...
Ulam module capabilities beyond the basic scalar model:

- **SUR** (seemingly unrelated regressions) — `MatrixPrior`, `CovMatrixPrior`, `UlamColumn.realMatrix`, per-row `multi_normal` loop emission.
- **Multivariate hierarchical priors** (cafe-style) — `LKJCorrCholeskyPrior`, `VaryingVectorPrior`, `.multivariateNormalCholesky`, `.lkjCorrCholesky`, `diag_pre_multiply` emission.
- **`dwishart` prior** — `WishartPrior`, `.wishart` Distribution, `wishartScaleColumns` exclusion from cardinality binding.
- **`dmvnormchol` / `dmvnorm2` alist aliases** — `ab[cafe] ~ dmvnorm2(c(a_bar, b_bar), sigma, L_Omega)` and the `dmvnormchol` arg-order variant route to `VaryingVectorPrior` + `LKJCorrCholeskyPrior` + `diag_pre_multiply`.
- **`'` (prime / transpose) token** — `AlistLexer` tokenises `'` as `.prime`; `parseBracketVectorArg` accepts `[id1, id2, …]'` dist args; `AlistTextEmitter` re-emits the Cholesky varying-vector form as `dmvnorm2(...)` (no `'` in output) so the alist text round-trips cleanly.
- **Gaussian process priors** — `GaussianProcessPrior`, squared-exponential `cov_GPL2` kernel, non-centred Cholesky form in `transformed parameters`.
- **Ordered logit / probit** — `OrderedCutpoints`, `Distribution.orderedLogistic` / `.orderedProbit`, `ordered[K-1] cutpoints` declaration.
- **Monotonic effects** (`mo()`) — `MonotonicEffect`, `SimplexPrior`, `Distribution.dirichlet`, per-row sum augmentation loop.
- **Nested groupings** — `NestedVaryingPrior("a", indexedBy: ["country", "region"], ...)`, `matrix[N_country, N_region] a`, `to_vector(a) ~ normal(...)` flat prior, `a[country[i], region[i]]` loop emission.
- **Crossed random effects with correlations** — two independent `VaryingVectorPrior` / `LKJCorrCholeskyPrior` / `VectorPrior` triples from distinct cardinality `.scalarInt` entries.
- **`sim()` / `generated_quantities`** — `Sim` DSL node + `<name> <- sim(d*(...))` alist syntax → `Statement.generatedQuantity` → Stan `generated quantities {}` block via `BlockEmitter.generated_QuantitiesBlock`; `generated_quantities` CLI subcommand runs cmdstan and merges per-chain `<name>_gq_[1..n].csv` → `<name>.generated_quantities.csv`.
- **`start=` / `constraints=` overrides** — `Constraints(lower:, upper:)` for declaration-only bounds; per-prior `start: Double?` merges into `Inits([:])`; `Inits([...])` DSL node writes `Results/<name>.init.json` consumed by `StanSample` as `init=<path>`.
- **`stan2alist` reverse pipeline** — full round-trip incl. multivariate (M1–M6): `StanBlockParser` → `StanToUlamModel` → `AlistTextEmitter`; `generated quantities` parsed and reverse-mapped to `sim()`; `transformed *` dropped with warning.
- **Robustness**: `countSymbol` collision checks; index-column value validation; per-row binomial outcome bounds (`transformed data` rejection block); bare-deterministic alist (identity link `mu <- a + b*x`); compound distribution-arg expressions (`.expression(String)`); string-column auto-factorisation in `csv2json` (writes `<name>.factors.json`); `Distribution` label ergonomics (labelled args for ambiguous cases); runinfo-first chain enumeration; dynamic `num_chains` via glob (supports single-chain and 10+ chain runs).
